...
class FramecastSeeder:
    ...
    async def clear_existing_data(self):
        ...
        try:
            ...
            await self.conn.execute("DELETE FROM job_events WHERE job_id IN (SELECT id FROM jobs WHERE owner LIKE 'framecast:%test%')")
            await self.conn.execute("DELETE FROM webhook_deliveries")
            await self.conn.execute("DELETE FROM jobs WHERE owner LIKE 'framecast:%test%'")
            await self.conn.execute("DELETE FROM asset_files WHERE owner LIKE 'framecast:%test%'")
            await self.conn.execute("DELETE FROM webhooks WHERE team_id IN (SELECT id FROM teams WHERE slug LIKE '%test%')")
            await self.conn.execute("DELETE FROM projects WHERE team_id IN (SELECT id FROM teams WHERE slug LIKE '%test%')")
            await self.conn.execute("DELETE FROM invitations WHERE team_id IN (SELECT id FROM teams WHERE slug LIKE '%test%')")
            await self.conn.execute("DELETE FROM api_keys WHERE owner LIKE 'framecast:%test%'")
            await self.conn.execute("DELETE FROM memberships WHERE team_id IN (SELECT id FROM teams WHERE slug LIKE '%test%')")
            await self.conn.execute("DELETE FROM teams WHERE slug LIKE '%test%'")
            await self.conn.execute("DELETE FROM users WHERE email LIKE '%@test.framecast.dev'")
    ...
    async def seed_teams(self, user_ids: List[str]) -> List[str]:
        ...
        for team in teams:
            ...
            """, team['id'], team['name'], team['slug'],
                team['credits'], json.dumps(team['settings']))

            # Add owner membership
            await self.conn.execute("""
                INSERT INTO memberships (id, team_id, user_id, role)
                VALUES ($1, $2, $3, 'owner')
            """, str(uuid.uuid4()), team['id'], team['owner_id'])
        ...
        await self.conn.execute("""
            INSERT INTO memberships (id, team_id, user_id, role)
            VALUES ($1, $2, $3, 'member')
        """, str(uuid.uuid4()), team_ids[0], user_ids[2])

        print(f"✅ Created {len(teams)} test teams with memberships")
        return team_ids

    async def seed_projects(self, team_ids: List[str], user_ids: List[str]) -> List[str]:
        ...
        for project in projects:
            await self.conn.execute("""
                ...
                VALUES ($1, $2, $3, $4, $5, $6)
            """, project['id'], project['team_id'], project['created_by'],
                project['name'], project['status'], json.dumps(project['spec']))
            project_ids.append(project['id'])

        print(f"✅ Created {len(projects)} test projects")
        return project_ids

    async def seed_jobs(self, team_ids: List[str], user_ids: List[str], project_ids: List[str]):
        """Create test jobs"""
        jobs = [
            {
                'id': str(uuid.uuid4()),
        ...
        for job in jobs:
            ...
            """, job['id'], job['owner'], job['triggered_by'], job['project_id'],
                job['status'], json.dumps(job['spec_snapshot']), job['credits_charged'],
                job.get('credits_refunded', 0), job.get('failure_type'),
                json.dumps(job.get('error')) if job.get('error') else None,
                job.get('completed_at'), job.get('started_at'))
    ...
    async def seed_api_keys(self, user_ids: List[str], team_ids: List[str]):
        ...
        for key in api_keys:
            ...
            """, key['id'], key['user_id'], key['owner'], key['name'],
                key['key_prefix'], key['key_hash'], json.dumps(['*']))
    ...
    async def seed_system_assets(self):
        ...
        for asset in system_assets:
            ...
            """, asset['id'], asset['category'], asset['name'], asset['description'],
                asset['duration_seconds'], asset['s3_key'], asset['content_type'],
                asset['size_bytes'], asset['tags'])
    ...
    async def seed_all(self, clear_existing: bool = False):
        ...
        try:
            ...
            print("\nTest Data Created:")
            print("  👥 Users: alice@test.framecast.dev (creator), bob@test.framecast.dev (starter), charlie@test.framecast.dev (creator)")
...
async def main():
    ...
    parser = argparse.ArgumentParser(description='Seed Framecast database with test data')
