...
class HealthChecker:
    ...
    async def check_database(self) -> bool:
        ...
        try:
            ...
            try:
                migrations = await conn.fetch("SELECT * FROM _sqlx_migrations ORDER BY version")
            ...
            self.results['services']['database'] = {
                ...
                'connection_url': DATABASE_URL.split('@')[1] if '@' in DATABASE_URL else 'configured'
            }
            return True
    ...
    async def check_localstack(self) -> bool:
        ...
        try:
            ...
            status = 'healthy' if (outputs_exists and assets_exists and can_write) else 'degraded'

            print(f"  {'✅' if status == 'healthy' else '⚠️'} LocalStack S3")
            print(f"    Outputs bucket: {'✅' if outputs_exists else '❌'} {S3_BUCKET_OUTPUTS}")
            print(f"    Assets bucket: {'✅' if assets_exists else '❌'} {S3_BUCKET_ASSETS}")
    ...
    async def check_external_apis(self) -> bool:
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get("https://api.anthropic.com", timeout=5) as response:
