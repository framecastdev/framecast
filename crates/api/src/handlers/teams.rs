...
use framecast_domain::entities::{MembershipRole, Team, UserTier};
...
fn validate_slug_format(slug: &str) -> std::result::Result<(), validator::ValidationError> {
    if crate::validation::validate_team_slug(slug) {
        Ok(())
    } else {
...
pub async fn create_team(
    ...
    let membership = framecast_domain::entities::Membership::new(
        created_team.id,
        user.id,
