...
for managing users, teams, generations, assets, and authentication.
...
- **4.8:** Added SSE reconnection protocol to GenerationEvent
...
- **8.5:** Added `list_generations`, `get_generation` operations
...
- **4.7:** Added `failure_type` and `credits_refunded` fields to Generation entity
- **8.2:** Added `create_team` operation
- **8.5:** Added `clone_generation` operation
- **9.1:** Added `POST /v1/teams` and `POST /v1/generations/:id/clone` endpoints
...
- **6 (all):** Complete rewrite of Invariants to match entity definitions exactly
  ...
  - Fixed Generation status: `processing` (was incorrectly `running`), `canceled` (was `cancelled`)
  - Fixed ApiKey field names: `key_hash` (was `hashed_key`), `revoked_at` (was `is_active`)
  - Removed references to non-existent fields (`email_verified`, `team_id` on Generation, etc.)
...
- **8.5:** Added complete Generation Operations (list/get/create_ephemeral/create_project/events/cancel/delete/clone)
...
- **4.7:** Added `updated_at` field and ON UPDATE trigger to Generation entity
...
- **Entity Model:** User, Team, Generation, Asset, Invitation, and SystemAsset entities
- **State Machines:** State transitions for Generations, Invitations, and other entities
...
- **v0.0.1-SNAPSHOT (Current):** January 30, 2025
  - Entity completeness: Generation/AssetFile updated_at, Team.name, validate_spec
- **v0.4.2:** January 30, 2025
  - Spec integrity pass: fixed invariants, added webhook/API key operations
- **v0.4.1:** January 30, 2025
  - Added team creation, generation cloning, team limits, credit refund policy
- **v0.4.0:** January 2025
