...
Once a user/team reaches their concurrent job limit, additional job submissions will be queued and executed when resources become available.
...
When the pending invitation limit is reached, additional invitations cannot be sent until existing invitations are accepted or revoked. The daily rate limit is reset at UTC midnight.
