...
class MigrationTestFramework:
    ...
    async def run_migrations(self) -> Dict[str, Any]:
        ...
        try:
            ...
            ], capture_output=True, text=True, env=env, cwd='/Users/thiago/Workscape/splice')
...
async def test_happy_01_clean_migration_from_scratch(migration_framework):
    ...
    core_tables = ['users', 'teams', 'memberships', 'projects', 'jobs',
                  'job_events', 'asset_files', 'webhooks', 'api_keys']
...
async def test_happy_02_migration_status_tracking(migration_framework):
    ...
    for migration in migrations:
        assert migration['success'], f"Migration {migration['version']} should be successful"
...
async def test_error_01_database_connection_failure():
    ...
    assert 'connection' in result.get('stderr', '').lower() or 'connection' in result.get('error', '').lower(), \
